...
- [x] 4.1 确认问题：`actionAmount` 初始 0，输入框永远显示 "0"，删不掉
- [x] 4.2 确认影响范围：仅 `CompanyPage.tsx:895`，`TradeForm.tsx` 用字符串状态无此问题
- [x] 4.3 输入框 `value={actionAmount}` → `value={actionAmount || ''}`，初始显示空
- [x] 4.4 验证：初始为空，输入 "123" 正常，清空后为空不弹回 "0"（需手动验证）
